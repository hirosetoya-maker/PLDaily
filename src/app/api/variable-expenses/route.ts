...
export async function POST(req: NextRequest) {
  ...
  const rows = await sql`
    INSERT INTO variable_expenses (user_id, name, amount, payment_date, month, is_paid)
    VALUES (${session.user.id}, ${name}, ${amount}, ${paymentDate}::date, ${month}::date, ${isPaid ?? false})
    RETURNING id
  `
